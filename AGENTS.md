...
Ball is a freestanding C++20 container/utility library (namespace `BTL`, optional public C++20 modules `Ball.Types` / `Ball.Time`). Public headers live under `include/ball/`; the few compiled sources and handwritten test module interfaces live under `src/ball/`; generated public and component interfaces are declared in `cmake/ball/modules.cmake` and `cmake/ball/types/modules.cmake`; tests live under `src/ball/types/tests/`.
