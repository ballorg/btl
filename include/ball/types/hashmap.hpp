...
/// probe chains gap-free by backward-shift deletion (@ref CMultiHashMapImpl::Remove)
...
// same reflect-tag idiom `CMultiRBTree` applies to its links. Value columns are
// stored as supplied -- give identically-typed values distinct types yourself
// (e.g. via `BALL_REFLECT_TAGGED`), exactly as `CMultiRBTree` requires.
...
/// metadata column, all held by a `CBufferMultiVector` sized to the bucket count
...
class CMultiHashMapBase : public C, public CBufferMultiVector< I, N, TI, EHashSlotState, HashKeyColumn_t< K >, Ts... >
{
public:
	...
	using Base_t = CBufferMultiVector< I, N, TI, EHashSlotState, HashKeyColumn_t< K >, Ts... >;
	...
	static constexpr TI PAYLOAD_COLUMN_OFFSET = static_cast< TI >( Storage_t::NUM_TYPES - COLUMN_COUNT );
	...
	// Access to the hash-policy empty-base subobject, mirroring CMultiRBTree's
	...
	constexpr Index_t Count() const noexcept
	{
		...
		const uchar_t *pStateBits = Nodes().template PackedBaseBy< State_t >();
		ullong_t nLive = 0;
	...
	constexpr CMultiHashMapBase() noexcept : CMultiHashMapBase( Hasher_t() ) {}

	constexpr explicit CMultiHashMapBase( const Hasher_t &hasher ) noexcept :
		Hasher_t( hasher ),
		Storage_t()
	{
	}

	~CMultiHashMapBase() noexcept = default;
...
protected:
	...
	{
		return Storage_t::template PackedGetDataBitBy< State_t >( pStateBits, Storage_t::template PackedBitOffsetBy< State_t >( i ) );
	}
	...
	constexpr void PlaceAt( Index_t i, Us &&...values )
	{
	...
	void ReinsertRow( Storage_t &temp, Index_t r, MIndexSequence< size_t, Is... > )
	{
		...
		PlaceAt( iSlot, Move( ReflectAccess( Get< PAYLOAD_COLUMN_OFFSET + static_cast< TI >( Is ) >( temp )[ r ] ) )... );
	}
...
///-----------------------------------------------------------------------------
/// @brief Public unique-key facade over `CMultiHashMapBase`.
///
...
/// row, mirroring `CMultiRBTreeImpl`.
///-----------------------------------------------------------------------------
template < typename I, I N, typename TI, typename C, typename K, typename... Ts >
class CMultiHashMapImpl : public CMultiHashMapBase< I, N, TI, C, K, Ts... >
{
private:
	using Base_t = CMultiHashMapBase< I, N, TI, C, K, Ts... >;
	using ColumnSequence_t = MakeIndexSequence_t< size_t, 1 + sizeof...( Ts ) >;

	using Base_t::LocateForInsert;
	using Base_t::PlaceAt;
...
public:
	...
	using iterator = CSlotIterator< CMultiHashMapImpl, false >;
	using const_iterator = CSlotIterator< CMultiHashMapImpl, true >;
	...
	constexpr Index_t Insert( K key, Ts... values )
	{
		...
		for ( ;; )
		{
			...
			{
				PlaceAt( iSlot, Move( key ), Move( values )... );
...
public:
	...
	template < I ON >
	constexpr CMultiHashMapImpl &CopyFrom( const CMultiHashMapImpl< I, ON, TI, C, K, Ts... > &other )
	{
	...
	template < I ON >
	constexpr CMultiHashMapImpl &MoveFrom( CMultiHashMapImpl< I, ON, TI, C, K, Ts... > &&other )
	{
		if ( reinterpret_cast< const void * >( this ) == reinterpret_cast< const void * >( &other ) )
			return *this;
	...
	template < I ON > constexpr CMultiHashMapImpl( const CMultiHashMapImpl< I, ON, TI, C, K, Ts... > &other ) { CopyFrom( other ); }
	template < I ON > constexpr CMultiHashMapImpl &operator=( const CMultiHashMapImpl< I, ON, TI, C, K, Ts... > &other ) { return CopyFrom( other ); }
	template < I ON > constexpr CMultiHashMapImpl( CMultiHashMapImpl< I, ON, TI, C, K, Ts... > &&other ) { MoveFrom( Move( other ) ); }
	template < I ON > constexpr CMultiHashMapImpl &operator=( CMultiHashMapImpl< I, ON, TI, C, K, Ts... > &&other ) { return MoveFrom( Move( other ) ); }
	...
	constexpr CMultiHashMapImpl( const CMultiHashMapImpl &other ) { CopyFrom( other ); }
	constexpr CMultiHashMapImpl &operator=( const CMultiHashMapImpl &other ) { return CopyFrom( other ); }
	constexpr CMultiHashMapImpl( CMultiHashMapImpl &&other ) { MoveFrom( Move( other ) ); }
	constexpr CMultiHashMapImpl &operator=( CMultiHashMapImpl &&other ) { return MoveFrom( Move( other ) ); }
};
...
class CMultiHashMap : public CMultiHashMapImpl< I, 0, size8_t, C, K, Ts... >
{
public:
	using Base_t = CMultiHashMapImpl< I, 0, size8_t, C, K, Ts... >;
	using Base_t::Base_t;
	using Base_t::CopyFrom; using Base_t::MoveFrom;

	/// @complexity O(m + n): cross-form copy/move conversions rebuild the table.
	template < I N > constexpr CMultiHashMap( const CBufferMultiHashMap< I, N, K, C, Ts... > &other ) { CopyFrom( other ); }
	template < I N > constexpr CMultiHashMap &operator=( const CBufferMultiHashMap< I, N, K, C, Ts... > &other ) { return CopyFrom( other ); }
	template < I N > constexpr CMultiHashMap( CBufferMultiHashMap< I, N, K, C, Ts... > &&other ) { MoveFrom( Move( other ) ); }
	template < I N > constexpr CMultiHashMap &operator=( CBufferMultiHashMap< I, N, K, C, Ts... > &&other ) { return MoveFrom( Move( other ) ); }
};

/// @brief Fixed-capacity (inline buffer) counterpart of `CMultiHashMap`. @p N is
...
class CBufferMultiHashMap : public CMultiHashMapImpl< I, N, size8_t, C, K, Ts... >
{
public:
	using Base_t = CMultiHashMapImpl< I, N, size8_t, C, K, Ts... >;
	using Base_t::Base_t;
	using Base_t::CopyFrom; using Base_t::MoveFrom;

	constexpr CBufferMultiHashMap( const CMultiHashMap< I, K, C, Ts... > &other ) { CopyFrom( other ); }
	constexpr CBufferMultiHashMap &operator=( const CMultiHashMap< I, K, C, Ts... > &other ) { return CopyFrom( other ); }
	constexpr CBufferMultiHashMap( CMultiHashMap< I, K, C, Ts... > &&other ) { MoveFrom( Move( other ) ); }
	constexpr CBufferMultiHashMap &operator=( CMultiHashMap< I, K, C, Ts... > &&other ) { return  MoveFrom( Move( other ) ); }
};

///-----------------------------------------------------------------------------
/// @brief Single-value hash map: key @p K, value @p V. A thin spelling of the
/// multi-column core with exactly one value column. @p N is the inline-buffer
/// capacity: `0` (the default) is heap-backed, a positive power of two keeps up
/// to @p N buckets inline before spilling to the heap.
///-----------------------------------------------------------------------------
template < typename I = size32_t, typename K = I, typename V = K, typename C = CFibonacciHash< I >, I N = 0 >
class CHashMap : public CMultiHashMapImpl< I, N, size8_t, C, K, V >
{
public:
	using Base_t = CMultiHashMapImpl< I, N, size8_t, C, K, V >;
	using Base_t::Base_t;
	using Base_t::Get;
	using Base_t::CopyFrom; using Base_t::MoveFrom;
	...
	/// @brief Accesses the key (column 0) or mapped value (column 1) by slot index.
	///
	/// @complexity O(1).
	...
	constexpr V &Value( Index_t i ) { return Base_t::template Get< 1 >( i ); }
	constexpr const V &Value( Index_t i ) const { return Base_t::template Get< 1 >( i ); }

	/// @complexity O(m + n): each rebuilds the table via CopyFrom/MoveFrom.
	template < I ON > constexpr CHashMap( const CHashMap< I, K, V, C, ON > &other ) { CopyFrom( other ); }
	template < I ON > constexpr CHashMap &operator=( const CHashMap< I, K, V, C, ON > &other ) { return CopyFrom( other ); }
	template < I ON > constexpr CHashMap( CHashMap< I, K, V, C, ON > &&other ) { MoveFrom( Move( other ) ); }
	template < I ON > constexpr CHashMap &operator=( CHashMap< I, K, V, C, ON > &&other ) { return MoveFrom( Move( other ) ); }
};

/// Convenience spellings that fix the index width and forward the hash policy @p C.
/// @p C defaults to `CFibonacciHash< I >`, so the hash-word width travels with the
/// map (it matches the index width, which bounds the bucket count). A custom
/// policy can be supplied when a different hashing strategy is required.
///
/// Single-value map: key @p K + value @p V.
template < typename I = size32_t, typename K = I, typename V = K, typename C = CFibonacciHash< I > > using HashMap_t = CHashMap< I, K, V, C >;
template < typename K, typename V = K, typename C = CFibonacciHash< size8_t > > using HashMap8_t = CHashMap< size8_t, K, V, C >;
template < typename K, typename V = K, typename C = CFibonacciHash< size16_t > > using HashMap16_t = CHashMap< size16_t, K, V, C >;
template < typename K, typename V = K, typename C = CFibonacciHash< size32_t > > using HashMap32_t = CHashMap< size32_t, K, V, C >;
template < typename K, typename V = K, typename C = CFibonacciHash< size64_t > > using HashMap64_t = CHashMap< size64_t, K, V, C >;

template < size_t N, typename K = size_t, typename V = K, typename C = CFibonacciHash< size_t > > using BufferHashMap_t = CHashMap< size_t, K, V, C, N >;
template < size8_t N, typename K = size8_t, typename V = K, typename C = CFibonacciHash< size8_t > > using BufferHashMap8_t = CHashMap< size8_t, K, V, C, N >;
template < size16_t N, typename K = size16_t, typename V = K, typename C = CFibonacciHash< size16_t > > using BufferHashMap16_t = CHashMap< size16_t, K, V, C, N >;
template < size32_t N, typename K = size32_t, typename V = K, typename C = CFibonacciHash< size32_t > > using BufferHashMap32_t = CHashMap< size32_t, K, V, C, N >;
template < size64_t N, typename K = size64_t, typename V = K, typename C = CFibonacciHash< size64_t > > using BufferHashMap64_t = CHashMap< size64_t, K, V, C, N >;
...
template < typename K, typename C = CFibonacciHash< size32_t >, typename... Ts > using MultiHashMap_t = CMultiHashMap< size32_t, K, C, Ts... >;
template < typename K, typename C = CFibonacciHash< size8_t >, typename... Ts > using MultiHashMap8_t = CMultiHashMap< size8_t, K, C, Ts... >;
template < typename K, typename C = CFibonacciHash< size16_t >, typename... Ts > using MultiHashMap16_t = CMultiHashMap< size16_t, K, C, Ts... >;
template < typename K, typename C = CFibonacciHash< size32_t >, typename... Ts > using MultiHashMap32_t = CMultiHashMap< size32_t, K, C, Ts... >;
template < typename K, typename C = CFibonacciHash< size64_t >, typename... Ts > using MultiHashMap64_t = CMultiHashMap< size64_t, K, C, Ts... >;
