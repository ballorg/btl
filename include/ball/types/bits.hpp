...
#	if defined( BALL_MSVC )
	if constexpr ( NUM_BITS <= 32 )
	{
		const uint_t nMask = static_cast< uint_t >( x - !!x );
		ulong_t i; // Bit index returned by the scan intrinsic.

		_BitScanReverse( &i, nMask | !nMask );

		return static_cast< I >( i + !!nMask );
	}
	else if constexpr ( NUM_BITS <= 64 )
	{
		const ullong_t nMask = static_cast< ullong_t >( x - !!x );
		ulong_t i; // Bit index returned by the scan intrinsic.

		_BitScanReverse64( &i, nMask | !nMask );
...
#	elif defined( BALL_GNUC )
	...
	if constexpr ( NUM_BITS <= 32 )
	{
		...
		return static_cast< I >( 31u - static_cast< uint_t >( __builtin_clz( nMask | !nMask ) ) + !!nMask );
	}
	else if constexpr ( NUM_BITS <= 64 )
	{
		const ullong_t nMask = static_cast< ullong_t >( x - !!x );

		return static_cast< I >( 63u - static_cast< uint_t >( __builtin_clzll( nMask | !nMask ) ) + !!nMask );
	}
	else
		return INVALID;
...
template < typename I >
constexpr I BitCeil_Unified( I x ) noexcept
{
	constexpr I NUM_BITS = MFixed< I >::BITS;
...
/// @tparam I Unsigned integral type described by MFixed<I>.
/// @param x Input value.
/// @return I(1) << BitWidth(x) for supported widths, otherwise MFixed<I>::INVALID.
template < typename I >
constexpr I BitCeil( I x )
{
	if constexpr ( MFixed< I >::BITS <= 64 )
		return I( 1 ) << BitWidth( x );
	else
		return MFixed< I >::INVALID;
}
