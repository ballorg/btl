...
- `Insert( key, values... )` — rejects duplicates (`NIL_INDEX`); its hot path performs probing and placement in one loop, keeping the packed-state and key-column bases hoisted through the operation and prefetching the initial probe bucket. Growth is triggered lazily without a size field: only a probe reaching `PROBE_GROW_THRESHOLD` (8) pays for the exact popcount load check (`ShouldGrow`, target load 3/4); a completely full cycle grows unconditionally.
- `Find` / `Contains` / `FindIterator` — probe until match or `FREE`; while a buffered map remains at `N`, lookup selects its packed-state and key inline bases together, prefetches the initial state bit and key, and uses them directly without repeated inline/heap resolution in the probe path.
- `Remove( key )` — backward-shift deletion (Knuth 6.4R): prefetches the state and key of the next cluster bucket, then slides trailing entries whose home bucket permits it back over the hole, keeping chains contiguous with no tombstones.
