...
- `Find` / `Contains` / `FindIterator` — probe until match or `FREE`; while a buffered map remains at `N`, lookup selects its packed-state and key inline bases together and uses them directly without repeated inline/heap resolution in the probe path. Linear probing relies on the processor's sequential-access prefetcher instead of issuing a software hint immediately before each load.
