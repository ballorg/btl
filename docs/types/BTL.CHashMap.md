...
Bucket columns follow the [multi-column CVector storage model](BTL.CVector.md): inline column buffers up to `N` rows (`CBufferHashMap`), then one shared heap block. The vector row count is the active bucket count; `GrowTable` doubles the geometric capacity reported by the inherited `Capacity()`. A power-of-two `N >= 8` is the buffered form's reserve mechanism: construction activates those `N` inline rows directly through the inherited `SetCount`, and the first insertion uses them without `ResetTable` or intermediate rehashes. Resizing likewise delegates allocation, SoA pointer updates, and row lifetime management to `CBufferVector::SetCount` and its internal `EnsureCapacity` path. When a non-empty active table grows, live rows are snapshotted into an always-heap-backed scratch multi-column `CVector` using its optimized `SetCountRaw` path, so a large `CBufferHashMap` does not duplicate its inline capacity on the stack. The table is then sized to the new power-of-two bucket count and each row is re-probed into its new home. The packed state column is cleared as one byte range rather than one slot at a time. The table never shrinks on removal.
...
- `Find` / `Contains` / `FindIterator` — probe until match or `FREE`; lookup obtains the tagged key column through `CBufferVector::BaseBy` and reads occupancy through the base packed-bit primitive, avoiding generic column-index/proxy resolution in the hot path. The state check remains before the random key load, which is faster for this SoA layout. While a buffered map remains at `N`, lookup selects its packed-state and key inline bases together. Linear probing relies on the processor's sequential-access prefetcher instead of issuing a software hint immediately before each load.
- `Remove( key )` — backward-shift deletion (Knuth 6.4R): slides trailing entries whose home bucket permits it back over the hole, keeping chains contiguous with no tombstones.
...
Iteration order is bucket order, which is a function of hashing — not insertion or key order. For the inline `CBufferHashMap`, choose `N` as a power of two ≥ 8 to actually stay inline. The convenience aliases fix the hash policy to `CFibonacciHash< I >`; instantiate `CHashMap` or `CBufferHashMap` directly when a custom policy is required. The Release comparison in `case11_mapbenchmark.cpp` executes 32 find passes for both hash tables inside each timed trial and normalizes the reported duration to one pass, reducing noise without changing the per-lookup workload.
